            ...
            <div class="bg-white shadow-xl rounded-lg overflow-hidden">
                ...
                <div class="p-6">
                        ...
                        <div>
                            ...
                            <div class="space-y-3 pl-7">
                                ...
                                <div class="flex justify-between">
                                    ...
                                    <span class="font-medium">${{ number_format($bookingDetails['price_per_night'], 2) }}</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="mt-10 flex flex-col sm:flex-row justify-end gap-4">
                        ...
                        <form action="{{ route('bookings.saveForLater') }}" method="POST" class="w-full sm:w-auto">
                            @csrf
                            @foreach($bookingDetails as $key => $value)
                                <input type="hidden" name="{{ $key }}" value="{{ $value }}">
                            @endforeach
                            <button type="submit" class="w-full px-6 py-3 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all">
                                <div class="flex items-center justify-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <form action="{{ route('bookings.store') }}" method="POST" class="w-full sm:w-auto">
                            @csrf
                            @foreach($bookingDetails as $key => $value)
                                <input type="hidden" name="{{ $key }}" value="{{ $value }}">
                            @endforeach
