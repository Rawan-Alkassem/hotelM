<x-app-layout>
    ...
    <h2 class="font-semibold text-xl text-red-600 leading-tight text-left mb-6">
        Rooms List
    </h2>
    ...
    <div class="py-6 bg-gray-900 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 text-left">
            ...
            @if (session('success'))
                <div class="mb-4 font-medium text-sm text-green-500 bg-green-900 px-4 py-2 rounded">
                    {{ session('success') }}
                </div>
            @endif
                        ...
                        @forelse($rooms as $room)
                            <tr class="hover:bg-gray-700">
                                ...
                                <td class="px-4 py-2 space-x-2 whitespace-nowrap">
                                    <a href="{{ route('rooms.show', $room->id) }}" class="text-blue-400 hover:underline text-sm">View</a>
                                    <a href="{{ route('rooms.edit', $room->id) }}" class="text-yellow-400 hover:underline text-sm ms-3">Edit</a>
                                    <form action="{{ route('rooms.destroy', $room->id) }}" method="POST" class="inline ms-3" onsubmit="return confirm('Are you sure?');" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-500 hover:underline text-sm bg-transparent border-0 p-0">Delete</button>
                                    </form>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-4 text-center text-gray-400">No rooms found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
